...
import { LayoutDashboard, Database, Network, Search, Plus, Boxes, ChevronRight, ChevronDown, Edit2, LayoutGrid, List, Filter, X, Settings, Map as MapIcon, Layers } from 'lucide-react';
...
const InventoryView = ({ apps, onRefresh }: { apps: Application[], onRefresh: () => void }) => {
  ...
  const [filters, setFilters] = useState({ search: '', owner: '', lifecycle: '', type: '' });
  ...
  const rangeMetaFields = metaDefs?.filter(d => d.fieldType === 'range') || [];
  ...
  const clearFilters = () => setFilters({ search: '', owner: '', lifecycle: '', type: '' });
      ...
      {showFilters && (
        <div className="card" style={{ marginBottom: '2rem', background: 'var(--background)', padding: '1rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', alignItems: 'flex-end' }}>
            <div className="field" style={{ margin: 0 }}><label className="label">Search</label><input value={filters.search} onChange={(e) => setFilters({...filters, search: e.target.value})} placeholder="Name or Desc..." style={{ marginTop: '0.25rem' }} /></div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem', alignItems: 'flex-start' }}>
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px' }}>
                      {rangeMetaFields.map(def => {
                        const val = meta[def.fieldName] ?? def.min;
                        return (
                          <div 
                            key={def.id} 
                            title={`${def.label}: ${val}`}
...
const CapabilitiesView = ({ onRefresh }: { onRefresh: () => void }) => {
  ...
  const capabilityTree = useMemo(() => {
    ...
    map.forEach(cap => { if (cap.parentId && map.has(cap.parentId)) map.get(cap.parentId)!.children!.push(cap); else roots.push(cap); });
    return roots;
  }, [flatCapabilities]);
...
const DiagramsView = ({ apps, onEditApp, onEditCapability }: { apps: Application[], onEditApp: (app: Application) => void, onEditCapability: (cap: any) => void }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
      ...
      {showFilters && (<div style={{ padding: '1rem 2rem', borderBottom: '1px solid var(--border)', background: 'var(--card)', flexShrink: 0 }}><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', alignItems: 'flex-end' }}><div className="field" style={{ margin: 0 }}><label className="label">Search</label><input value={filters.search} onChange={(e) => setFilters({...filters, search: e.target.value})} placeholder="Search..." style={{ marginTop: '0.25rem' }} /></div><div className="field" style={{ margin: 0 }}><label className="label">Owner</label><select value={filters.owner} onChange={(e) => setFilters({...filters, owner: e.target.value})} style={{ marginTop: '0.25rem' }}><option value="">All Owners</option>{ownerOptions.map((o: any) => <option key={o.id} value={o.value}>{o.label}</option>)}</select></div><div className="field" style={{ margin: 0 }}><label className="label">Type</label><select value={filters.type} onChange={(e) => setFilters({...filters, type: e.target.value})} style={{ marginTop: '0.25rem' }}><option value="">All Types</option>{appTypeOptions.map((o: any) => <option key={o.id} value={o.value}>{o.label}</option>)}</select></div><div className="field" style={{ margin: 0 }}><label className="label">Lifecycle</label><select value={filters.lifecycle} onChange={(e) => setFilters({...filters, lifecycle: e.target.value})} style={{ marginTop: '0.25rem' }}><option value="">All Lifecycles</option>{lifecycleOptions.map((opt: any) => (<option key={opt.id} value={opt.value}>{opt.label}</option>))}</select></div><button onClick={() => setFilters({ search: '', owner: '', lifecycle: '', type: '' })} style={{ height: '2.5rem', borderColor: 'transparent', color: 'var(--muted-foreground)' }}><X size={16} style={{ marginRight: '0.5rem' }} /> Clear</button></div></div>)}
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}><ApplicationDiagram onNodeClick={onEditApp} onCapabilityClick={onEditCapability} appsOverride={filteredApps} mode={mode} activeOverlay={activeOverlay} /></div>
    </div>
  );
};
