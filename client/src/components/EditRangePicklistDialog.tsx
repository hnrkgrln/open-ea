...
export const EditRangePicklistDialog = ({ picklist, onSuccess }: Props) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/picklists/${picklist.id}/options`, {
        ...
        body: JSON.stringify(options.map(({ id, ...rest }) => rest)),
      });
    ...
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <button className="secondary" style={{ height: '2rem', padding: '0 0.75rem', fontSize: '0.75rem' }}>
          <Sliders size={14} style={{ marginRight: '0.5rem' }} /> Configure Range
        </button>
      </Dialog.Trigger>
      <Dialog.Portal>
        ...
        <Dialog.Content style={{ 
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
            <Dialog.Title style={{ fontWeight: 600 }}>Configure Range: {picklist.label}</Dialog.Title>
          ...
          <Dialog.Description style={{ marginBottom: '1.5rem', fontSize: '0.875rem', color: 'var(--muted-foreground)' }}>
            Quickly generate a numeric range with semantic colors or edit individual steps.
          </Dialog.Description>
          ...
          <div style={{ marginBottom: '2rem' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--border)' }}>
                  <th style={{ padding: '0.5rem', fontSize: '0.7rem', textTransform: 'uppercase' }}>Value</th>
                  <th style={{ padding: '0.5rem', fontSize: '0.7rem', textTransform: 'uppercase' }}>Label</th>
                ...
                {options.map((opt, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '0.5rem' }}>
                      <input value={opt.value} onChange={e => updateOption(i, 'value', e.target.value)} style={{ width: '60px', margin: 0, fontSize: '0.875rem' }} />
                    </td>
                    <td style={{ padding: '0.5rem' }}>
                      <input value={opt.label} onChange={e => updateOption(i, 'label', e.target.value)} style={{ margin: 0, fontSize: '0.875rem' }} />
                    </td>
                    <td style={{ padding: '0.5rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <input type="color" value={opt.color} onChange={e => updateOption(i, 'color', e.target.value)} style={{ width: '30px', height: '30px', padding: 0, border: 'none', background: 'none' }} />
                        <span style={{ fontSize: '0.75rem', fontFamily: 'monospace' }}>{opt.color}</span>
                      </div>
                    </td>
                    <td style={{ padding: '0.5rem' }}>
                      <button onClick={() => setOptions(options.filter((_, idx) => idx !== i))} style={{ border: 'none', background: 'transparent', color: 'var(--destructive)' }}><Trash2 size={14} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {options.length === 0 && (
              <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--muted-foreground)', fontSize: '0.875rem' }}>No steps defined. Use the generator above.</div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
            ...
            <button onClick={handleSave} className="primary" disabled={loading || options.length === 0}>
              {loading ? 'Saving...' : 'Save Range Configuration'}
            </button>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
